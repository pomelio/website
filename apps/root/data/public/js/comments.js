$(document).on('my-account', (event, account) => {
    ...
    function render(comment_id) {
        let html = `
    <button
        class="comment-button-${comment_id} inline-flex items-center p-2 text-sm font-medium text-center text-gray-500 dark:text-gray-400 bg-white rounded-lg hover:bg-gray-100 focus:ring-4 focus:outline-none focus:ring-gray-50 dark:bg-gray-900 dark:hover:bg-gray-700 dark:focus:ring-gray-600"
        type="button">
    ...
    if (account) {
        $(".comment-buttons").each(function(i, obj){
            ...
            if (user_id == account.id) {
                ...
                $('#comment-button-' + comment_id).click(e => {
                    $('#dropdown-comment-' + comment_id).toggleClass('hidden');
                });
